 ...
 * @returns void
 */
async function searchElements (words: string, logicalSwitch: boolean): Promise<void> {
 ...
 * @returns void
 */
async function searchAndDecryptElements (words: string, role: string, logicalSwitch: boolean): Promise<void> {
  const result = document.getElementById('result')
  const content = document.getElementById('content')
  if (result == null || content == null) {
    return
 ...
 */
function hybridDecryptionTest (abeUserDecryption: string, databaseEncryptedValue: string): number {
