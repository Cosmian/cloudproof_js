...
export function fromTTLV<T>(ttlv: TTLV, tag: string | null = null, siblings: TTLV[] = []): T {
    ...
    if (ttlv.type === TtlvType.Enumeration) {
        ...
        if (typeof ttlv.value !== "string") {
            throw new Error(`${ttlv.value.toString()} of type Enumeration should be encoded in string`);
        }
    ...
    if (ttlv.tag === "Object" && Array.isArray(ttlv.value) && ttlv.type === TtlvType.Structure) {
        // Object is a little special since we need to check a sibling ObjectType to know which struct to construct.

        if (siblings.length === 0) {
            throw new Error("To parse an Object, we need to have access to the list of siblings.");
        }

        const objectType = siblings.find((element) => element.tag === "ObjectType");
        if (objectType === undefined) {
            throw new Error("To parse an Object, we need to have an ObjectType in the siblings");
        }

        const result = { type: objectType.value, value: fromTTLV(ttlv, objectType.value as string) }
        return result as T
    }

    if (ttlv.tag === "KeyMaterial" && Array.isArray(ttlv.value) && ttlv.type === TtlvType.Structure) {
        // KeyMaterial is a little special since we need to check a sibling ObjectType to know which struct to construct.

        if (siblings.length === 0) {
            throw new Error("To parse a KeyMaterial, we need to have access to the list of siblings.");
        }

        const attributes = siblings.find((element) => element.tag === "Attributes");
        if (attributes === undefined) {
            throw new Error("To parse a KeyMaterial, we need to have an Attributes in the siblings");
        }

        if (! Array.isArray(attributes.value) || attributes.type !== 'Structure') {
            throw new Error("During parse of a KeyMaterial, the Attributes in the siblings should be a structure");
        }

        const keyFormatType = attributes.value.find((element) => element.tag === "KeyFormatType");
        if (keyFormatType === undefined) {
            throw new Error("To parse a KeyMaterial, we need to have a KeyFormatType in the Attributes in the siblings");
        }
        ...
        throw new Error(`Cannot understand the KeyMaterial inside ${JSON.stringify(ttlv)} with siblings ${JSON.stringify(siblings)}`)
    }

    if (ttlv.tag in STRUCTS && Array.isArray(ttlv.value) && ttlv.type === TtlvType.Structure) {
        // @ts-expect-error
        ...
        for (const child of ttlv.value) {
            ...
            if (Array.isArray(instance[propertyName])) {
                if (! Array.isArray(child.value)) {
                    throw new Error(`Property ${propertyName as string} of ${typeof instance} should be an array but ${JSON.stringify(child)} value is not an array.`)
                }
    ...
    throw new Error(`Cannot understand the KMIP struct inside ${JSON.stringify(ttlv, null, 4)}`)
} 

export type Serializable = {
    tag: string
} | string | number | BigInt | Serializable[]
 ...
 */
export function toTTLV(kmip: Serializable | string, tag: string | null = null): TTLV {
    if (typeof kmip === "string") {
        if (tag === null) throw new Error(`Trying to TTLV '${kmip}' but no tag provided.`)

        const type = (tag in ENUMS || tag === "ObjectType") ? TtlvType.Enumeration : TtlvType.TextString;
        return { tag, type, value: kmip }
    }

    if (typeof kmip === "boolean") {
        if (tag === null) throw new Error(`Trying to TTLV '${kmip ? 'true' : 'false'}' but no tag provided.`)
    ...
    if (typeof kmip === "number") {
        ...
        const type = (tag in ENUMS || tag in ENUMS_FLAGS) ? TtlvType.Enumeration : TtlvType.Integer;
        ...
        if (value === undefined) throw new Error(`Value is undefined for enum ${kmip} (enum type ${tag})`)
        return { tag, type, value }
    }

    if (typeof kmip === "bigint" || kmip instanceof BigInt) {
        if (tag === null) throw new Error(`Trying to TTLV '${kmip.toString()}' but no tag provided.`)
        return { tag, type: TtlvType.BigInteger, value: `0x${kmip.toString(16).toUpperCase()}` }
    }
    ...
    if (Array.isArray(kmip)) {
        if (tag === null) throw new Error(`Trying to TTLV an array but no tag provided.`)
        return { 
            tag,
    ...
    const ttlv = {
        ...
        value: Object.entries(kmip)
            ...
            .filter(([, propertyValue]) => propertyValue !== null && (! Array.isArray(propertyValue) || propertyValue.length))
