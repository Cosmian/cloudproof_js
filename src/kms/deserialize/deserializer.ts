...
export class FromTTLV {
  public static array<T extends Object>(propertyName: string, ttlv: TTLV, elementMetadata: PropertyMetadata): T[] {
    // check TTLV type
    const ttlvType = ttlv.type
    if (ttlvType && ttlvType !== TtlvType.Structure) {
      throw new Error('Invalid type: ' + ttlvType + ' for structure ' + ttlv.tag)
    }
    ...
    for (const v of ttlvValue) {
      if (v.tag !== ttlv.tag) {
        throw new Error('Invalid child with name' + v.tag + ' in array of ' + ttlv.tag)
      }
  ...
  private static parseValue (ttlv: TTLV, metadata: PropertyMetadata, propertyName: string): any {
    ...
    if (metadata.type === TtlvType.Choice) {
      ...
      throw new Error('The child ' + propertyName + ' is a TTLV Choice but there is no deserializer in the Metadata')
    }
    ...
    if (ttlv.type === TtlvType.BigInteger) {
      ...
      throw new Error('The child ' + propertyName + ' is a TTLV Enumeration but there is no enum Metadata')
    } else if (ttlv.type === TtlvType.Integer) {
      try {
        return parseInt(ttlv.value as string, 10)
      } catch (error) {
        throw new Error('The child ' + propertyName + ' is a TTLV Integer but its value: ' + ttlv.value + ' cannot be parsed as an Integer')
      }
      ...
      throw new Error('The child ' + propertyName + ' is a TTLV Structure but there is no deserializer in the Metadata')
  ...
  public static structure<T extends Object>(type: new(...args: any[]) => T, ...args: any[]): (propertyName: string, ttlv: TTLV) => T {
    return (propertyName: string, ttlv: TTLV): T => {
      ...
      if (expectedName !== actualName) {
        throw new Error('The expected structure name: ' + expectedName + 'in ' + propertyName + ', does not match the actual name: ' + actualName)
      }

      // check TTLV type
      const ttlvType = ttlv.type
      if (ttlvType && ttlvType !== TtlvType.Structure) {
        throw new Error('Invalid type: ' + ttlvType + ' for structure ' + actualName + ' in ' + propertyName)
      }

      // check value is array
      if (ttlv.value.constructor.name !== 'Array') {
        throw new Error('Invalid value for structure ' + actualName + 'in ' + propertyName + ': it should be an array')
      }
      const ttlvValue = ttlv.value as TTLV[]
  ...
  public static choice<T extends Object>(type: new(...args: any[]) => T, ...args: any[]): (propertyName: string, ttlv: TTLV) => T {
    return (propertyName: string, ttlv: TTLV): T => {
      ...
      throw new Error('Choice of type' + ttlvType + ' not found for object ' + instance.constructor.name + ' in ' + propertyName)
    }
  }
}
