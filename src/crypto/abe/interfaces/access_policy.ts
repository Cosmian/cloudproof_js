...
export class AccessPolicy {
    ...
    public toVendorAttribute(): VendorAttribute {
        return new VendorAttribute(VendorAttribute.VENDOR_ID_COSMIAN, VendorAttribute.VENDOR_ATTR_COVER_CRYPT_ACCESS_POLICY, new TextEncoder().encode(this.toKmipJson()))
    }
    ...
    public static fromAttributes(attributes: Attributes): AccessPolicy {
        ...
        for (const att of attrs) {
            if (att.attribute_name === VendorAttribute.VENDOR_ATTR_COVER_CRYPT_ACCESS_POLICY || att.attribute_name === VendorAttribute.VENDOR_ATTR_ABE_ACCESS_POLICY) {
                return AccessPolicy.fromKmipJson(new TextDecoder().decode(att.attribute_value))
            }
        }
        throw new Error(`No access policy available in the vendor attributes`)
    }
...
function toBooleanExpression_(obj: Object, depth: number): string {
    ...
    if (op.toLowerCase() === "and") {
        return `${leftBracket}${toBooleanExpression_(next[0], depth + 1)} && ${toBooleanExpression_(next[1], depth + 1)}${rightBracket}`
    }
