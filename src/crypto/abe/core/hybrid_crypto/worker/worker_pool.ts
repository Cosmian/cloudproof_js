...
export interface EncryptedEntry { uidHex: string, ciphertextHex: string }
...
export class WorkerPool {
  ...
  public async decrypt (decryptionKeyHex: string, encryptedEntries: EncryptedEntry[], isGpswImplementation: boolean): Promise<Uint8Array[]> {
    return await runWorkers(this.workers, decryptionKeyHex, encryptedEntries, isGpswImplementation)
  }
...
const runWorker = async (worker: Worker, decryptionKeyHex: string, encryptedEntries: EncryptedEntry[], isGpswImplementation: boolean): Promise<Uint8Array[]> => {
  // Transform asynchronous messaging wih callback into a Future
  return await new Promise((resolve: (value: Uint8Array[] | PromiseLike<Uint8Array[]>) => void, reject: (reason?: any) => void) => {
