...
export class PostgRestDB implements DBInterface {
  ...
  async getEntryTableEntriesById (uids: Uint8Array[]): Promise<Index[]> {
    ...
    const uidsAndValuesHex: IndexString[] = await this.instance.get(`/index_entry?uid=in.(${uidsHex.toString()})`).then(this.responseBody)
    return uidsAndValuesHex.map(element => new Index(hexDecode(element.uid), hexDecode(element.value)))
  }

  async getChainTableEntriesById (uids: Uint8Array[]): Promise<Index[]> {
    const uidsHex = uids.map(uid => hexEncode(uid))
    const uidsAndValuesHex: IndexString[] = await this.instance.get(`/index_chain?uid=in.(${uidsHex.toString()})`).then(this.responseBody)
    return uidsAndValuesHex.map(element => new Index(hexDecode(element.uid), hexDecode(element.value)))
  }

  async upsertEntryTableEntries (entries: Index[]): Promise<number> {
    const uidsAndValuesHex = entries.map(element => new IndexString(hexEncode(element.uid), hexEncode(element.value)))
  ...
  async getEncryptedUsersById (uids: string[]): Promise<EncryptedUser[]> {
    return await this.instance.get(`/encrypted_users?uid=in.(${uids.toString()})`).then(this.responseBody)
  }
  ...
  async upsertEncryptedUser (entry: EncryptedUser): Promise<Array<{ uid: string, enc_uid: string }>> {
    const config = {
      headers: {
