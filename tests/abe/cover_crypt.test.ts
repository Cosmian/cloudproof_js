...
test("Non regression tests", async () => {
  ...
  const { plaintext } = hybridDecryption.decrypt(ENCRYPTED_DATA)
...
test("Non regression tests from Java", async () => {
  ...
  const { plaintext } = hybridDecryption.decrypt(encryptedBytes)

  const plaintextDecoder = new TextDecoder().decode(plaintext)
  ...
  expect(plaintextDecoder).toEqual(expectedPlaintext)
})
...
async function runTests(policy: Policy): Promise<void> {
  ...
  const { plaintext } = hybridDecryptionMediumSecret.decrypt(lowSecretMkgData)
  expect(plaintext).toEqual(PLAINTEXT)
  ...
  {
    const { plaintext } = hybridDecryptionTopSecret.decrypt(lowSecretMkgData)
    expect(plaintext).toEqual(PLAINTEXT)
  }

  {
    const { plaintext } = hybridDecryptionTopSecret.decrypt(topSecretMkgData)
    expect(plaintext).toEqual(PLAINTEXT)
  }

  {
    const { plaintext } = hybridDecryptionTopSecret.decrypt(lowSecretFinData)
    expect(plaintext).toEqual(PLAINTEXT)
  }
}
